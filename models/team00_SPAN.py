    ...
    def update_params(self):
        ...
        with torch.no_grad():
            ...
            for g in range(self.groups):
                ...
                # 计算合并后的权重
                # import pdb; pdb.set_trace()
                w_g = F.conv2d(w1_g.flip(2, 3).permute(1, 0, 2, 3), w2_g, padding=2, stride=1).flip(2, 3).permute(1, 0,
                                                                                                                  2, 3)

                weight_concat_g = F.conv2d(w_g.flip(2, 3).permute(1, 0, 2, 3), w3_g, padding=0, stride=1).flip(2,
                                                                                                               3).permute(
                    1, 0, 2, 3)
                ...
                if b1 is not None and b2 is not None and b3 is not None:
                    b_g = (w2_g * b1[
                                  g * group_out_channels * self.gain:(g + 1) * group_out_channels * self.gain].reshape(
                        1, -1, 1, 1)).sum((1, 2, 3)) + \
                          b2[g * group_out_channels * self.gain:(g + 1) * group_out_channels * self.gain]
                    bias_concat_g = (w3_g * b_g.reshape(1, -1, 1, 1)).sum((1, 2, 3)) + b3[g * group_out_channels:(
                                                                                                                             g + 1) * group_out_channels]
...
class SlimBlock(nn.Module):
    def __init__(self, c):
        ...
        # dw_channel = c // 2
        dw_channel = c
        self.conv1 = nn.Conv2d(in_channels=dw_channel, out_channels=dw_channel, kernel_size=3, padding=1, stride=1,
                               groups=2, bias=True)
